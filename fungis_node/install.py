...
def install_agent_cli(
    *,
    ...
    executable_path = executable_path or Path.home() / ".local" / "bin" / "fungis"
    config_path = config_path or Path.home() / ".config" / "fungis" / "agent.json"
    executable_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.parent.mkdir(parents=True, exist_ok=True)
    wrapper = (
        "#!/bin/sh\n"
        f"exec {json.dumps(sys.executable)} -m fungis_node.agent_cli \"$@\"\n"
    )
