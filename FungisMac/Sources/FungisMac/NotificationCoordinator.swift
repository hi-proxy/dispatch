...
final class NotificationCoordinator: NSObject, UNUserNotificationCenterDelegate {
    ...
    func consume(_ snapshot: FungisSnapshot) {
    ...
    private func notifyPermissionRequests(_ snapshot: FungisSnapshot) {
        ...
        where !announcedPermissions.contains(request.id) {
            ...
            UNUserNotificationCenter.current().add(
                UNNotificationRequest(
                    identifier: "fungis-permission-\(request.id)",
    ...
    private func notifyAwaitingInput(_ snapshot: FungisSnapshot) {
        ...
        for agent in waiting where !announcedAwaiting.contains(agent.surfaceID) {
            ...
            UNUserNotificationCenter.current().add(
                UNNotificationRequest(
                    identifier: "fungis-awaiting-\(agent.surfaceID)",
                    content: content, trigger: nil
                )
            )
        }
    ...
    private func notify(_ message: ChatMessage) {
        ...
        let request = UNNotificationRequest(
            identifier: "fungis-message-\(message.seq)", content: content, trigger: nil
        )
