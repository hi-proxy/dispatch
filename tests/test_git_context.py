...
from dispatch_node.git_context import inspect_git_context, is_verified_commit
...
def test_git_context_reports_verified_branch_head_and_dirty_state(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    git(repo, "init", "-b", "feature/dispatch-tracks")
    ...
    assert clean["branch"] == "feature/dispatch-tracks"
    assert clean["branches"] == ["feature/dispatch-tracks"]
