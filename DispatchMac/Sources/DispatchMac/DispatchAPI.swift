...
enum DispatchAPIError: LocalizedError {
    case invalidResponse
    case server(Int, String)

    var errorDescription: String? {
        switch self {
        case .invalidResponse: "Invalid response from Dispatch"
        case let .server(code, detail): "Dispatch \(code): \(detail)"
        }
    }
}

struct DispatchAPI: Sendable {
    let baseURL: URL
    ...
    func snapshots(projectID: String = "local") -> AsyncThrowingStream<DispatchSnapshot, Error> {
            ...
            let reader = Task {
                ...
                do {
                    while !Task.isCancelled {
                        ...
                        @unknown default: throw DispatchAPIError.invalidResponse
                        }
                        continuation.yield(try JSONDecoder().decode(DispatchSnapshot.self, from: data))
                    }
                    continuation.finish()
                } catch {
    ...
    func createProject(name: String) async throws -> DispatchProject {
        struct Payload: Encodable { let name: String }
        return try await request("api/projects", method: "POST", body: Payload(name: name))
    }

    func updateProject(id: String, name: String) async throws -> DispatchProject {
        struct Payload: Encodable { let name: String }
    ...
    private func request<Response: Decodable>(
        ...
        guard let http = response as? HTTPURLResponse else {
            throw DispatchAPIError.invalidResponse
        }
        guard 200..<300 ~= http.statusCode else {
            let detail = (try? JSONDecoder().decode(ErrorBody.self, from: data).detail)
                ?? String(data: data, encoding: .utf8) ?? "unknown error"
            throw DispatchAPIError.server(http.statusCode, detail)
        }
